    ...
    >
      <el-row>
        ...
        <el-form-item class="select" v-if="type!='info'"  label="账号" prop="zhanghao">
          <el-select v-model="ruleForm.zhanghao" placeholder="请选择账号">
            <el-option
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                            ...
                                                            this.$http({
              url: `option/jiaowurenyuan/zhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                ...
                this.$message.error(data.msg);
                                        ...
                                        this.leixingOptions = "调课,停课".split(',')
                                                                                                                                        },
                                                                                // 多级联动参数
                                                                                                      info(id) {
      this.$http({
        url: `diaokexinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `diaokexinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.diaokexinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
