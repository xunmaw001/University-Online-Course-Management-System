          ...
          <el-form-item class="input" label="课程名称" prop="kechengmingcheng">
              <el-input v-model="ruleForm.kechengmingcheng" 
                placeholder="课程名称" readonly></el-input>
                                    ...
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.jiankongzhaopian" label="监控照片" prop="jiankongzhaopian">
          <file-upload
          tip="点击上传监控照片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.jiankongzhaopian?ruleForm.jiankongzhaopian:''"
          @change="jiankongzhaopianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.jiankongzhaopian" label="监控照片" prop="jiankongzhaopian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jiankongzhaopian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="24">  
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kechengmingcheng = true;
            continue;
	            	            ...
	            	            if(o=='jiankongzhaopian'){
            this.ruleForm.jiankongzhaopian = obj[o];
	    this.ro.jiankongzhaopian = true;
            continue;
          }
	            	            if(o=='shipin'){
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `kaohejilu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.kaohejiluCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    back() {
      ...
      this.parent.contentStyleChange();
                                    ...
                                    jiankongzhaopianUploadChange(fileUrls) {
                this.ruleForm.jiankongzhaopian = fileUrls;
				this.addEditUploadStyleChange()
            },
