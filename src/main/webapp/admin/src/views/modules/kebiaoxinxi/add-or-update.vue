        ...
        <div v-else>
          <el-form-item v-if="ruleForm.tupian" label="图片" prop="tupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.tupian.split(',')" :src="item" width="100" height="100">
                                    ...
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="课表文件" prop="kebiaowenjian">
          <file-upload
          tip="点击上传课表文件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.kebiaowenjian?ruleForm.kebiaowenjian:''"
          @change="kebiaowenjianUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.kebiaowenjian" label="课表文件" prop="kebiaowenjian">
            <el-button type="text" size="small" @click="download(ruleForm.kebiaowenjian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
                                                                  <el-col :span="12">
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tupian = true;
            continue;
	            	            ...
	            	            if(o=='kebiaowenjian'){
            this.ruleForm.kebiaowenjian = obj[o];
	    this.ro.kebiaowenjian = true;
            continue;
          }
	            	            if(o=='neirong'){
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `kebiaoxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.kebiaoxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    back() {
      ...
      this.parent.contentStyleChange();
				...
				this.addEditUploadStyleChange()
                                ...
                                kebiaowenjianUploadChange(fileUrls) {
                this.ruleForm.kebiaowenjian = fileUrls;
				this.addEditUploadStyleChange()
            },
                                                            	addEditStyleChange() {
	  this.$nextTick(()=>{
